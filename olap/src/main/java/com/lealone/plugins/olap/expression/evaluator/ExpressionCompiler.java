...
public class ExpressionCompiler extends VoidExpressionVisitor {
    ...
    public static synchronized JitEvaluator createJitEvaluator(HotSpotEvaluator evaluator,
            ServerSession session, Expression expression) {
        ...
        buff.append("package ").append(Constants.USER_PACKAGE).append(".expression.evaluator")
                .append(";\r\n\r\n");
    ...
    private static JitEvaluator loadFromSource(ServerSession session, StringBuilder buff,
    ...
    public static synchronized void createJitEvaluatorAsync(HotSpotEvaluator evaluator,
            ServerSession session, Expression expression,
    ...
    private ExpressionCompiler(HotSpotEvaluator evaluator, StringBuilder buff, TreeSet<String> importSet,
            int level, String retVar) {
    ...
    public Void visitOperation(Operation e) {
        ...
        if (ltype != dataType)
            buff.append("    ").append(indent).append(retVarLeft).append(" = ").append(retVarLeft)
        ...
        case Operation.CONCAT:
            ...
            buff.append("Operation.concat(").append(retVarLeft).append(", ").append(retVarRight)
                    .append(", ").append(mode.nullConcatIsNull).append(");\r\n");
            break;
        default:
            importSet.add(ValueNull.class.getName());
            buff.append(retVarLeft).append(" == ValueNull.INSTANCE || ").append(retVarRight)
                    .append(" == ValueNull.INSTANCE ? ValueNull.INSTANCE : ").append(retVarLeft)
                    .append(".").append(opTypeName).append("(").append(retVarRight).append(")")
                    .append(";\r\n");
        }
    ...
    public Void visitParameter(Parameter e) {
        ...
        if (value == null || value == ValueNull.INSTANCE) {
            ...
            buff.append(indent).append(retVar).append(" = evaluator.getValue(")
                    .append(evaluator.getValueListSize() - 1).append(");\r\n");
        }
        return null;
    }
    ...
    public Void visitRownum(Rownum e) {
        ...
        buff.append(indent).append(retVar).append(
                " = ValueInt.get(((StatementBase)session.getCurrentCommand()).getCurrentRowNumber());\r\n");
        return null;
    }
    ...
    public Void visitVariable(Variable e) {
        StringBuilder indent = indent((level + 1) * 4);
        buff.append(indent).append(retVar).append(" = session.getVariable(\"").append(e.getName())
                .append("\");\r\n");
        return null;
    }
    ...
    public Void visitComparison(Comparison e) {
        ...
        if (rtype != dataType)
            buff.append("    ").append(indent).append(retVarRight).append(" = ").append(retVarRight)
                    .append(".convertTo(").append(dataType).append(");\r\n");
        buff.append("    ").append(indent)
                .append("boolean result = Comparison.compareNotNull(session.getDatabase(), ")
                .append(retVarLeft).append(", ").append(retVarRight).append(", ")
                .append(e.getCompareType()).append(");\r\n");
    ...
    public Void visitConditionAndOr(ConditionAndOr e) {
        ...
        case ConditionAndOr.AND:
            buff.append("    ").append(indent).append("if (!").append(retVarLeft)
                    .append(".getBoolean()) {\r\n");
            buff.append("    ").append(indent).append("    ").append(retVar).append(" = ")
                    .append(retVarLeft).append(";\r\n");
            buff.append("    ").append(indent).append("} else {\r\n");
            right.accept(copy(level + 2, retVarRight));
            buff.append("    ").append(indent).append("    if (!").append(retVarRight)
                    .append(".getBoolean()) {\r\n");
            buff.append("    ").append(indent).append("        ").append(retVar).append(" = ")
                    .append(retVarRight).append(";\r\n");
    ...
    public Void visitConditionInConstantSet(ConditionInConstantSet e) {
        ...
        buff.append("    ").append(indent).append("    ").append(retVar).append(" = ").append(retVarLeft)
                .append(";\r\n");
        buff.append("    ").append(indent).append("} else if (evaluator.getValueSet().contains(")
                .append(retVarLeft).append(")) {\r\n");
        buff.append("    ").append(indent).append("    ").append(retVar)
                .append(" = ValueBoolean.TRUE;\r\n");
        buff.append("    ").append(indent).append("} else {\r\n");
        buff.append("    ").append(indent).append("    ").append(retVar);
